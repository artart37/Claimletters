...
export const selectedItemAnime = trigger('selectanime', [
  state('not-added', style('*')),
  state('added', style('*')),
  transition('not-added => added', [
    query('.simple-text', animate('0.1s linear', style({ opacity: 0 })), {
      optional: true,
    }),
    query(
      '.checkmark',
    ...
    query('.simple-text', animate('0.3s 0.2s ease-in', style({ opacity: 1 })), {
      optional: true,
    }),
  ]),
]);
...
export const howItWorks = trigger('howItWorksanime', [
  state('none', style('*')),
  state('start', style({ flexWrap: 'wrap' })),
  state('final', style({ flexWrap: 'wrap' })),

  transition('none => start', [
    //Animating the first two items seperately one after another - leaving
    query(
      '.how-it-works-section:nth-of-type(1)',
      animate(
        '0.3s 2s ease-out',
        style({
          right: '50%',
    ...
    query(
      '.how-it-works-section:nth-of-type(2)',
      animate(
        '0.3s 0.3s ease-out',
        style({
          right: '100%',
    ...
    group([
      query(
        '.how-it-works-section:nth-of-type(n+2):nth-of-type(odd)',
        stagger('3.2s', [
          animate(
            '0.3s 0.3s ease-out',
            style({
      ...
      query(
        '.how-it-works-section:nth-of-type(n+4):nth-of-type(even)',
        stagger('3.2s', [
          animate(
            '0.3s 0.6s ease-out',
            style({
  ...
  transition('start => final', [
    group([
      ...
      query(
        '.how-it-works-section:nth-of-type(even)',
